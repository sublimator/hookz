...
Found downstream: `seen_ids[i]` displayed as `seen_ids` is valid-looking C
and a lie — the reader has no way to tell that a subscript was eaten, so the
panel reports a line of code that is not in the file.
"""
