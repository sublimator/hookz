...
def str_concat(
    ...
    """Concatenate two null-terminated strings, write result to buffer.

    This is a simplified version of xahaud str_concat that handles the
    string concatenation case (operand_type == 6). The C++ version supports
    concatenating strings with integers, floats, etc. via operand_type.

    Here we implement the common case: two memory regions containing
    null-terminated strings, concatenated into the write buffer with a
    null terminator.

    Returns total length written (including null terminator).
    """
    if write_len > 1024 or read1_len > 1024:
        return hookapi.TOO_BIG

    if write_len == 0 or read1_len == 0:
        return hookapi.TOO_SMALL

    if write_len < read1_len:
        return hookapi.TOO_SMALL
...
def str_replace(
    ...
    haystack = rt._read_memory(haystack_ptr, haystack_len)
    needle = rt._read_memory(needle_ptr, needle_len)
    replacement = rt._read_memory(replacement_ptr, replacement_len)

    pos = haystack.find(needle)
    if pos == -1:
        return hookapi.DOESNT_EXIST

    result = haystack[:pos] + replacement + haystack[pos + needle_len :]
