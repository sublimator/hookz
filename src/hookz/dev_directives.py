...
/* Observe a named value at this point in execution. */
#define HOOKZ_U64(name, value) hookz_dev_u64((uint32_t)(name), HOOKZ_DEV_STRLEN(name), (uint64_t)(value))
#define HOOKZ_I64(name, value) hookz_dev_i64((uint32_t)(name), HOOKZ_DEV_STRLEN(name), (int64_t)(value))
#define HOOKZ_BYTES(name, ptr, len) hookz_dev_bytes((uint32_t)(name), HOOKZ_DEV_STRLEN(name), (uint32_t)(ptr), (uint32_t)(len))

/* Close a checkpoint: everything observed since the last one becomes a single
   named record. Recording only — nothing is asserted here. */
#define HOOKZ_CHECK(tag) hookz_dev_check((uint32_t)(tag), HOOKZ_DEV_STRLEN(tag))
