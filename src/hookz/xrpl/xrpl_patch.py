"""Monkey-patch xrpl-py definitions with Xahau server definitions.

Usage:
    from hookz.xrpl_patch import patch_xahau_definitions
    patch_xahau_definitions()

    from xrpl.core.binarycodec import decode
    decoded = decode(txn_hex)  # now understands Remit, SetHook, etc.
"""
...
def patch_xahau_definitions() -> None:
    """Load bundled server_definitions.json and patch xrpl-py."""
    global _patched
    if _patched:
        return
    import json
    ...
    defs_path = Path(__file__).parent / "server_definitions.json"
    server_defs = json.loads(defs_path.read_text())
